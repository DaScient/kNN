...
"""names=['Detection', 'Detection_int', 'Sensor',, 'tach0' 'tach1', 'tach2', 'tach3', 'tach4', 
...
'tach90']"""
df = pd.read_csv('mex_ld_lu2.csv', header=0)
""", names=names)"""
...
X = np.array(df.ix[:,3:93]) 	# end index is exclusive (Tachs 0-90)
y = np.array(df['Detection_int'])
Xnew = SelectKBest(chi2, k=23).fit_transform(X, y) #Best-Feature-Selection. 
 	# another way of indexing a pandas df
